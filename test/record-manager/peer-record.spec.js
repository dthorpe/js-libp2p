...
describe('PeerRecord', () => {
  ...
  it('isEqual returns false if the peer record has a different peerId', async () => {
    ...
    const peerRecord1 = new PeerRecord(peerId1)
  ...
  it('isEqual returns false if the peer record has a different seqNumber', () => {
    const ts0 = Date.now()
    const peerRecord0 = new PeerRecord(peerId, [], ts0)

    const ts1 = ts0 + 20
    const peerRecord1 = new PeerRecord(peerId, [], ts1)
  ...
  it('isEqual returns false if the peer record has a different multiaddrs', () => {
    ...
    const peerRecord1 = new PeerRecord(peerId, multiaddrs1)
